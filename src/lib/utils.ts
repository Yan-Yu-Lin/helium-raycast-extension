...
export function formatTimestamp(timestamp: number): string {
  // Helium uses WebKit timestamp (microseconds since Jan 1, 1601)
...
export async function handleError(error: unknown, context?: string): Promise<void> {
  ...
  if (error instanceof Error) {
    const heliumError = error as HeliumError;

    if (heliumError.code === "APPLESCRIPT_ERROR") {
      await showToast({
        style: Toast.Style.Failure,
        title: "Connection Error",
        message: "Unable to communicate with Helium browser",
      });
    } else if (heliumError.code === "TAB_SWITCH_ERROR") {
      await showToast({
        style: Toast.Style.Failure,
